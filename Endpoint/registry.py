...
def _extract_trigger_examples(markdown_text: str) -> list[str]:
    """
    Pulls the bullet list out of an _index.md's '### Trigger Examples'
    body section (present in every exporter today, but not required).

    main_SKILL.md Section 3 STEP 3 says routing should use "each
    candidate's purpose and trigger_examples (if available)" -- this is
    the "if available" source. Every _index.md already writes this
    section; previously nothing ever read it back out, so Phase 1 was
    routing on a one-line `purpose` string and the short exact-phrase
    `trigger_keywords` list alone, with none of the richer illustrative
    examples ever reaching the router.
    """
    ...
    for line in lines:
        stripped = line.strip()
        if stripped.startswith("### Trigger Examples"):
            in_section = True
            continue
...
def scan_skills_folder(skills_root: str) -> list[dict]:
    ...
    for entry_name in sorted(os.listdir(skills_root)):
        ...
        entries.append({
            ...
            "index_dir": subdir,
            "trigger_examples": _extract_trigger_examples(text),
            **frontmatter,
        })
...
def format_registry_for_prompt(entries: list[dict]) -> str:
    ...
    for e in entries:
        ...
        examples = e.get("trigger_examples") or []
        lines.append(
        ...
        if examples:
            ...
            lines.append(f"  trigger_examples (illustrative, not exhaustive):\n{example_lines}")
    return "\n".join(lines)
