...
async def intent_router(user_question: str) -> dict:
    ...
    - Match the question against each exporter's trigger_keywords first (whole-word/phrase match). If none match
      exactly, fall back to each exporter's purpose and trigger_examples above -- a question can plausibly belong
      to an exporter even when it doesn't share an exact keyword phrase with anything listed, as long as its
      intent is clearly the same kind of thing the purpose/trigger_examples describe (e.g. a specific GPU
      compute-pipeline question belongs to an exporter whose purpose/examples cover general GPU compute activity,
      even if that precise pipeline name never appears in the keyword list). Only return routing_type = "unmapped"
      if, after checking keywords, purpose, AND trigger_examples, no exporter plausibly covers it.
...
async def query_generator(user_question: str, routing_data: dict, domain_data: dict) -> str:
    ...
    3. A domain file's "Query Examples" section saying "no verified example available, do not invent a literal
       ...
       states construction cannot proceed / is blocked. Delegating construction to "the Main Skill and Database
       Fundamentals" (as many metrics do) means proceed with their standard rules, not refuse.
       If, and only if, that field explicitly blocks construction (e.g. because the exposed unit is
       unverified): do NOT build a `query` field at all, and resolve that measurement as
       `status: "unsupported_metric"` (never `"ok"`) with `requested_measurement` naming the metric you
       correctly identified, and `explanation` restating the field's own stated reason. The metric being
       correctly identified does not make the result `"ok"` -- identification and constructability are
       separate questions, and a block on the latter always wins.
...
async def validation_tester(user_question: str, routing_data: dict, domain_data: dict,
    ...
    11. Does the selected metric's own "Metric-Specific Query/Result Semantics" field in the domain content
        above explicitly state that construction is blocked / cannot proceed for it? If so, the result MUST be
        `status: "unsupported_metric"` with no `query` field. A metric matching this condition that was
        instead given `status: "ok"` and a constructed query is a FAIL regardless of whether that query's
        PromQL syntax is otherwise correct -- a syntactically valid query built on blocked semantics is still
        a fabrication of the one thing (the semantics) that was explicitly withheld.
